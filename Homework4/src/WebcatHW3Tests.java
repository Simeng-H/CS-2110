...
public class WebcatHW3Tests {
    ...
    public void testEraseEmpty() {
        ...
        assertFalse("PhotoLibrary.removePhoto() failed (edge case)", b.removePhoto(new Photograph("Caption","Filename")));
    }
    ...
    public void testEraseThere() {
        ...
        assertFalse("PhotoLibrary.removePhoto() failed (something wasn't removed) (also check getPhotos())", b.getPhotos().contains(new Photograph("C2","F2")));
        Album a = null;
        for (Album c: b.getAlbums()) {
            if (c.getName().equals("Vacation"))
                a = c;
        }
        assertEquals("PhotoLibrary.removePhoto() failed (photo wasn't removed from the albums) (also check getAlbums())", new ArrayList<Photograph>(), a.getPhotos());
        a = null;
        for (Album c: b.getAlbums()) {
            if (c.getName().equals("Vacation2"))
                a = c;
        }
        assertFalse("PhotoLibrary.removePhoto() failed (something wasn't removed from an album) (also check getPhotos())", a.getPhotos().contains(new Photograph("C2","F2")));
        assertTrue("PhotoLibrary.removePhoto() failed (removed too much from albums) (also check getPhotos())", a.getPhotos().contains(new Photograph("C1","F1")));
        assertTrue("PhotoLibrary.removePhoto() failed (removed too much from albums) (also check getPhotos())", a.getPhotos().contains(new Photograph("C3","F3")));
        assertEquals("PhotoLibrary removePhoto removed too much from albums", 2, a.getPhotos().size());
        assertTrue("PhotoLibrary.removePhoto() failed (removed too much) (also check getPhotos())", b.getPhotos().contains(new Photograph("C1","F1")));
        assertTrue("PhotoLibrary.removePhoto() failed (removed too much) (also check getPhotos())", b.getPhotos().contains(new Photograph("C3","F3")));
        assertEquals("PhotoLibrary removePhoto removed too much", 2, b.getPhotos().size());
    }
    ...
    public void testGetPhotosYear() {
        ...
        assertTrue("PhotoLibrary addPhoto did not add a photo", a.addPhoto(b));
        assertTrue("PhotoLibrary addPhoto did not add a photo", a.addPhoto(c));
        assertTrue("PhotoLibrary addPhoto did not add a photo", a.addPhoto(d));
